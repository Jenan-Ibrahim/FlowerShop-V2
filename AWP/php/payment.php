...
if ($_SERVER["REQUEST_METHOD"] == "POST" && is_valid_card_number($_POST['cardNumber'])) {
    $cookie_value = $_POST['cardNumber'] . "|" . $_POST['expiryMonth'] . "|" . $_POST['expiryYear'] . "|" . $_POST['cvCode'];
    setcookie("paymentInfo", $cookie_value, time() + (86400 * 30), "/");
    $confirmation_message = "Payment information saved successfully!";
}

function card_exists($conn, $cardNumber)
{
    $sql = "SELECT * FROM payment WHERE card_number = '$cardNumber'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && is_valid_card_number($_POST['cardNumber'])) {
    ...
    $result_user_id = $stmt_select_user_id->get_result();
    ...
    $sql_check = "SELECT * FROM payment WHERE card_number = '$cardNumber'";
    $result_check = $conn->query($sql_check);

    if ($result_check->num_rows > 0) {
        $confirmation_message = "Card number already exists";
    } else {
